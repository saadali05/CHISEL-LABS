...
class TestALU extends FreeSpec with ChiselScalatestTester{
    ...
    for ( i <- 0 until 100) {
        ...
        val opr = Random.nextInt (5) //since 12 is not exculded we need to add 13 here to include 12
        ...
        val result = aluop match {
            ...
            case ALU_SLT => if( src_a < src_b) 1 else 0 //using if else since the it give false in the output and we want int and toInt is depricaitede
